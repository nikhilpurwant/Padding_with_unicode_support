...
def getUTF8ByteCountForUnicodeText(input_string):
  '''Function calculates the string length by counting the bytes
  that will be used to store each of the string's characters. 
  This is imporant because just len(str) will return 1 for 
  the English character 'A' as well as Spanish character 'ñ' as well
  as Devnagari character 'अ'. 
  Ideally we should get value 1,2 and 3 respectively,
  so that the padding size can be determined properly by 
  the paddingLength function above
  
  This function was later replaced with a more concise code snippet.
  e.g. pad_len = paddingLength(len(bytes(str,'utf-8')), blocksize)
  But it provides a good understanding of how utf-8 encoding works.
  More details can be found on the following stackoverflow page.
  https://stackoverflow.com/questions/6240055/manually-converting-unicode-codepoints-into-utf-8-and-utf-16
  '''
  number_of_utf_bytes = 0
  for character in input_string:
     ord_value = ord(character)
     if ord_value <= 127:
        number_of_utf_bytes = number_of_utf_bytes + 1
     elif ord_value <= 2047:
        number_of_utf_bytes = number_of_utf_bytes + 2
     elif ord_value <= 65535:
        number_of_utf_bytes = number_of_utf_bytes + 3
     else:
        number_of_utf_bytes = number_of_utf_bytes + 4
  
  return number_of_utf_bytes
...
def appendCMSPadding(str, blocksize=AES_blocksize):
  ...
  pad_len = paddingLength(len(bytes(str,'utf-8')), blocksize)
...
def appendBitPadding(str, blocksize=AES_blocksize):
  ...
  pad_len = paddingLength(len(bytes(str,'utf-8')), blocksize) - 1
